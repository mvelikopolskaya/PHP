...
class User {
    ...
    public function __construct(string $name = null, string $lastName = null, string $birthday = null, int $id_user = null, string $login = null){
        $this->userName = $name;
    ...
    public static function validateRequestData(): bool{
        ...
        if(!(
            ...
            if(!Validator::dateMatch($birthday)) {
                $result =  false;
            }
    ...
    public static function isAdmin(?int $id) {
        if ($id > 0) {
            $sql = "SELECT * FROM user_roles WHERE role = 'admin' AND id_user = :id";
            $handler = Application::$storage->get()->prepare($sql);
            $handler->execute([
                'id' => $id
            ]);
